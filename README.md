...
### `start_booking.sh` 변경 사항 및 크론 설정 권장사항

- 스크립트가 크론 친화적으로 업데이트되었습니다 (2025-12-14):
	- `HOME`을 하드코딩하지 않고 동적으로 설정합니다 (`export HOME="${HOME:-/Users/$(id -un)}"`).
	- `PATH`를 기본적으로 설정하여 `command -v`가 cron 환경에서도 동작하도록 합니다.
	- 표준출력/표준에러를 `logs/cron.log`로 리다이렉트하도록 변경되어, cron에서 실행된 출력은 `logs/cron.log`에서 확인할 수 있습니다.
	- `VENV_PYTHON`의 기본값은 `./venv/bin/python3`입니다. 가상환경을 사용 중이면 각자 `VENV_PYTHON`을 명시하는 것을 권장합니다.

- 문제 징후 및 대응
	- 로그에 `[WARN] Python executable not found at ''`가 나오면 `VENV_PYTHON`이 비어있어 가상환경 파이썬을 찾지 못한 상태입니다. 이 경우 `start_booking.sh` 상단 주석의 예시를 따라 경로를 지정하세요.
	- 스크립트에 실행 권한이 있는지 확인하세요: `chmod +x start_booking.sh`
...
| 2025-12-14 | 4.5.0 | start_booking.sh 업데이트 - 동적 HOME 디렉토리 추적 |
